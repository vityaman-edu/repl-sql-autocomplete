 ...
 */
bool CodeCompletionCore::collectFollowSets(antlr4::atn::ATNState* s,
  ...
  for (antlr4::atn::ConstTransitionPtr& tp : s->transitions) {
    ...
    if (transition->getTransitionType() == antlr4::atn::TransitionType::RULE) {
      ...
      if (std::find(ruleStack.begin(), ruleStack.end(),
      ...
      if (!ruleFollowSetsIsExhaustive) {
        bool nextStateFollowSetsIsExhaustive
            = collectFollowSets(ruleTransition->followState, stopState,
                followSets, stateStack, ruleStack);
      ...
      if (checkPredicate(static_cast<const antlr4::atn::PredicateTransition*>(
              transition))) {
  ...
  while (statePipeline.size() > 0) {
    ...
    for (antlr4::atn::ConstTransitionPtr& transition :
      ...
      case antlr4::atn::TransitionType::RULE: {
        ...
        RuleEndStatus endStatus = processRule(ruleStartState,
      ...
      case antlr4::atn::TransitionType::WILDCARD: {
        if (atCaret) {
          if (!translateStackToRuleIndex(callStack)) {
            for (auto token : antlr4::misc::IntervalSet::of(
                     antlr4::Token::MIN_USER_TOKEN_TYPE, atn.maxTokenType)
                                  .toList()) {
...
void CodeCompletionCore::printDescription(size_t indentation,
    antlr4::atn::ATNState* state, std::string const& baseDescription,
    size_t tokenIndex) {
